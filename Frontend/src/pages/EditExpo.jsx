...
import { getExpoById, updateExpo } from "../services/api";
...
const EditExpo = () => {
  ...
  const { id } = useParams(); // Get expo ID from URL
  ...
  // Fetch expo data on component mount
  useEffect(() => {
    const fetchExpo = async () => {
      try {
        ...
        Object.keys(expoData).forEach(key => {
          if (key in expoSchema || key === 'description' || key === 'theme' || key === 'website' || key === 'registrationDeadline') {
            setValue(key, expoData[key]);
          }
        });
        
        // Set booths
        if (expoData.booths && Array.isArray(expoData.booths)) {
          setBooths(expoData.booths.map((booth, index) => ({
            id: booth._id || `booth-${index}`,
            number: booth.number || "",
            size: booth.size || "Standard",
            price: booth.price || "1000",
        ...
        setError("Failed to load expo. Please try again.");
      } finally {
        setFetching(false);
      }
    };
  ...
  // Update expo function
  const updateExpoAPI = async (expoData) => {
    setLoading(true);
    setError("");
    
    try {
      const response = await updateExpo(id, expoData);
      setSuccess("Expo updated successfully!");
      
      setTimeout(() => {
        navigate("/viewexpo", { state: { message: "Expo updated successfully" } });
      }, 1500);
      
      return response.data;
    } catch (error) {
      console.error("Error updating expo:", error);
      setError(error.message || "Failed to update expo. Please try again.");
      throw error;
    } finally {
      setLoading(false);
    }
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      await updateExpoAPI(expoData);
    } catch (error) {
      // Error is already set in updateExpoAPI function
    }
  };

  const steps = ['Expo Details', 'Booth Allocation', 'Review & Update'];
          ...
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
              <Typography variant="h6" sx={{ color: "#fff", fontWeight: 600 }}>
                Edit Booth Allocation ({booths.length} booths)
              </Typography>
              <Button
                variant="contained"
                startIcon={<AddIcon />}
  ...
  if (fetching) {
    return (
      <Box sx={{ display: "flex", background: "#0f172a", minHeight: "100vh", alignItems: "center", justifyContent: "center" }}>
        ...
        <Typography sx={{ color: '#fff', ml: 2 }}>Loading expo data...</Typography>
      </Box>
    );
  }
        ...
        <Box sx={{ maxWidth: '1200px', mx: 'auto' }}>
          ...
          <Box sx={{ mb: 4 }}>
            ...
            <Typography sx={{ color: "#94a3b8", fontSize: "16px" }}>
              Update expo details and booth allocation
            </Typography>
          </Box>
